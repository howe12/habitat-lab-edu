...
class Seq2SeqAgent(nn.Module):
    ...
    - Text encoder: 简化 LSTM
    - Action embed: 5 个动作的 embedding
    - Fuser: 2 层 LSTM
    ...
    def __init__(self, hidden_dim=256, num_actions=5, pretrained=True):
        ...
        self.rgb_encoder = nn.Sequential(*list(resnet.children())[:-1])  # 去掉 fc
        rgb_feat_dim = 2048

        # Text encoder (简化 LSTM)
        self.txt_encoder = nn.LSTM(
            input_size=300, hidden_size=hidden_dim, batch_first=True
        )

        # Action embedding
        self.action_embed = nn.Embedding(num_actions + 1, 32)  # +1 for padding
    ...
    def forward(self, rgb_seq, instr, prev_actions):
        """
        Args:
            rgb_seq: (B, T, 3, H, W) - RGB 序列
            instr: (B, L) - token 化的指令（简化版用 length=10 随机张量）
        ...
        # 2. 文本编码
        txt_feat, _ = self.txt_encoder(instr)  # (B, L, hidden_dim)
        txt_pool = txt_feat[:, -1:, :].expand(-1, T, -1)  # (B, T, hidden_dim)

        # 3. 动作 embedding
        act_feat = self.action_embed(prev_actions)  # (B, T, 32)

        # 4. 拼接 + LSTM 融合
        fused = torch.cat([rgb_feat, txt_pool, act_feat], dim=-1)  # (B, T, 2640)
        out, _ = self.fuser(fused)  # (B, T, hidden_dim)

        # 5. 三个预测头
        action_logits = self.action_head(out)  # (B, T, 5)
...
if __name__ == "__main__":
    ...
    instr = torch.randn(B, 10, 300).cuda()
    prev_actions = torch.zeros(B, T, dtype=torch.long).cuda()

    action_logits, prog, stop = model(rgb_seq, instr, prev_actions)
