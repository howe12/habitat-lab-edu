...
> Step1 只下载了测试数据。本章带你了解 Habitat 生态中所有可用的场景和任务数据集，
    它们的用途、大小、获取方式，以及正确的目录结构。
...
配置文件中的 `{split}` 模板在运行时会展开为 `train` / `val` / `test`，
例如：`data_path: "data/datasets/pointnav/gibson/v1/{split}/{split}.json.gz"`
...
如果直连很慢（国内），可以通过 HTTP_PROXY 加速：`export http_proxy=http://127.0.0.1:7897`
...
| 数据集 | 获取步骤 | 审批时间 | 注意事项 |
| --- | --- | --- | --- |
| **Gibson** | 1. 阅读 [使用协议](https://storage.googleapis.com/gibson_material/Agreement%20GDS%2006-04-18.pdf)

2. 发邮件给 Gibson 作者申请下载链接

3. 下载 `gibson_4+_assets.tar.gz` | 通常 1-3 天 | 仅限学术研究用途
不可商业使用 |
| **MatterPort3D** | 1. 访问 [MP3D 官网](https://niessner.github.io/Matterport/) 填写申请表

2. 提供 GitHub 账号 + 机构邮箱

3. 审核通过后获取 Matterport API Key

4. 使用官方 download_mp.py 脚本下载 | 通常 1-2 周 | 需要 **.edu 邮箱**（学术机构）
API Key 不可分享 |
| **HM3D** | 1. 访问 [Matterport 合作伙伴页面](https://matterport.com/partners/facebook)

2. 提交 Habitat HM3D 数据集申请

3. 审批通过后获得下载权限 | 1-4 周 | 是 Matterport 与 Facebook AI 的合作项目
审批较为严格 |
...
`RearrangeDatasetV0` 的 `__init__` 方法中（`habitat/datasets/rearrange/rearrange_dataset.py` 第58-69行）
会自动检测数据路径是否存在；如果不存在，则调用：
...
# robots/  urdf/  ...
     ...
     学后自检 — 你掌握了什么
        ...
        Q1：data/scene_datasets/ 和 data/datasets/ 的区别？
